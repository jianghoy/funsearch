...
def _write_python_file(file_name: str, code: str, func_to_run: str, test_input: str):
    with open(file_name, "w") as f:
        ...
        f.write(f"  {func_to_run}({test_input})")
...
class Sandbox:
    ...
    def run(
        ...
        host_file_name = f"tmp_{uuid.uuid4().hex}.py"
        volume_file_name = f"/tmp/{host_file_name}"
        ...
        # Function to stop the container after a timeout
        def stop_container():
        ...
        stop_thread = threading.Thread(target=stop_container)
        stop_thread.start()


# TODO: here it only has 1 sandbox, is there a way to just let it spawn a bunch of boxes
# and test around it?
class Evaluator:
