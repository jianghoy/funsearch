...
class Evaluator:
    ...
    async def analyse(
        ...
        while True:
            ...
            if item is None:
                break
            sample_function_body, island_id, version_generated = item
            ...
            print()
            print("sample function body start")
            print(sample_function_body)
            print("sample function body complete")
            print("New function output start")
            print(str(new_function))
            print("New function output complete")
            print()

            tasks = [
                asyncio.create_task(self._sandbox_run(test_input, program))
            ...
            # Process results
            for test_input, test_output, runs_ok in results:
                if (
                    runs_ok
                    ...
                    if not isinstance(test_output, (int, float)):
                        logging.error("@function.run did not return an int/float score.")
                        logging.error(f"test_input: {test_input}; test_output: {test_output}")
            ...
            if not skip_registering:
                ...
                await self._database.register_function(new_function, island_id, scores_per_test)

            self._queue.task_done()

    async def _sandbox_run(
        self, test_input: str, program: str
    ) -> tuple[str, int | float, bool]:
        test_output, runs_ok = await self._sandbox.run(
            program, self._function_to_run_name, test_input, self._timeout_seconds
        )
        print(
            f"test input: {test_input}; test output: {test_output}; runs ok: {runs_ok}"
        )
        return test_input, test_output, runs_ok
...
class Sandbox:
    ...
    async def run(
        ...
        await _wait_for_finish(container, timeout_seconds)
        ...
        if container.status != "exited":
            ...
            logging.error(
                f"CONTAINER {container.short_id} TIMED OUT! LOG: {container.logs().decode('utf-8')}"
            )
        ...
        if response["StatusCode"] != 0:
            ...
            logging.error(
                f"CONTAINER {container.short_id} FAILED! LOG: {container.logs().decode('utf-8')}"
            )
            container.remove(force=True)
            os.remove(host_file_name)
            return None, False
        ...
        try:
            if self._output_type == "float":
                ...
                raise NotImplementedError(
                    f"Output type {self._output_type} not implemented"
                )
        except ValueError:
...
async def _wait_for_finish(container, timeout):
